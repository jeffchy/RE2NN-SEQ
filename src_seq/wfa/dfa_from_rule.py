...
from automata_tools import BuildAutomata, Automata, DFAtoMinimizedDFA, NFAtoDFA, isInstalled, drawGraph
from pydash import uniq

from src_seq.rule_utils.rule_tokenizer import ruleParser
from src_seq.wfa.reverse_regex import reverse_regex
...
class NFAFromRegex:
    ...
    def buildNFA(self, rule: str, reversed: bool=False, lang: str='en') -> Automata:
        language = set()
        self.stack = []
        self.automata = []
        previous = self.initOperator
        ruleTokens = ruleParser(rule, lang)
...
def dfaFromRule(rule: str, reversed: bool=False, lang: str='en') -> Automata:
    # add lang
    nfa = NFAFromRegex().buildNFA(rule, reversed, lang)
